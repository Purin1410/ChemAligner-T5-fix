...
class T5Model(pl.LightningModule):
    """
    Clean LightningModule:
    - No config parsing here
    - No getattr(default) spaghetti
    - All hyperparams are injected into args in train.py
    """

    def __init__(self, args):
        super().__init__()
        self.args = args
        # HF tokenizer object should be assigned by train.py: model.tokenizer = tokenizer
        self.tokenizer = None
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        out = self.t5_model.forward_train(
            ...
            labels=labels,
            seq2seq_loss_weight=float(self.args.seq2seq_loss_weight),
            contrastive_loss_weight=float(self.args.contrastive_loss_weight),
        )
        return out.loss, out.logits, out.lm_loss, out.contrastive_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Flush when reaching target eval batch size
    ...
    def configure_optimizers(self):
        ...
        if opt_name == "sgd":
            ...
            # Default: AdamW for T5 style training
